/*
Retrieve all series whose ID_WIKIDATA in T_WC_TMDB_SERIE is a VALID QID but DIFFERENT
...
CE FICHIER EST LE JUMEAU RISQUE DE wikidata-id-serie-fix.sql. Les deux se partagent la
...
corroboration dans l'autre sens : que le QID candidat porte lui-meme, en P4983, le
...
/* Meme colonne que dans le fix, pour que les deux CSV se lisent cote a cote. Le
   garde 3 la contraint a valoir ID_SERIE, sauf dans le cas rare ou l'entite porte
   PLUSIEURS P4983 : la jointure retient celui qui corrobore, cette sous-requete
...
/* ⚠ GARDE 3 : LA CORROBORATION, ET C'EST CE QUI DISTINGUE CE FICHIER DU FIX.
   ...
   ID_SERIE un entier : c'est voulu, un CAST vers CHAR fabriquerait une valeur de
   collation utf8mb4_general_ci et leverait l'erreur 1267 contre une colonne en
   utf8mb4_unicode_ci. */
INNER JOIN T_WC_WIKIDATA_STATEMENT sc ON sc.ID_WIKIDATA = WS.ID_WIKIDATA
       AND sc.ID_PROPERTY = 'P4983'
...
INNER JOIN T_WC_WIKIDATA_EXTERNAL_ID_VALUE corrob ON corrob.ID_STATEMENT = sc.ID_STATEMENT
       AND corrob.VALUE_EXTERNAL_ID = S1.ID_SERIE
...
AND S1.ID_WIKIDATA REGEXP '^Q[0-9]+$'
AND S1.ID_WIKIDATA <> WS.ID_WIKIDATA
/*
AND S1.ADULT = 0
 */
