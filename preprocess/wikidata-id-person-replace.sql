/*
Retrieve all persons whose ID_WIKIDATA in T_WC_TMDB_PERSON is a VALID QID but DIFFERENT
...
CE FICHIER EST LE JUMEAU RISQUE DE wikidata-id-person-fix.sql. Les deux se partagent la
...
corroboration dans l'autre sens : que le QID candidat porte lui-meme, en P4985, le
...
/* Meme colonne que dans le fix, pour que les deux CSV se lisent cote a cote. Le
   garde 3 la contraint a valoir ID_PERSON, sauf dans le cas rare ou l'entite porte
   PLUSIEURS P4985 : la jointure retient celui qui corrobore, cette sous-requete
...
/* ⚠ GARDE 3 : LA CORROBORATION, ET C'EST CE QUI DISTINGUE CE FICHIER DU FIX.
   ...
   ID_PERSON un entier : c'est voulu, un CAST vers CHAR fabriquerait une valeur de
   collation utf8mb4_general_ci et leverait l'erreur 1267 contre une colonne en
   utf8mb4_unicode_ci. */
INNER JOIN T_WC_WIKIDATA_STATEMENT sc ON sc.ID_WIKIDATA = WP.ID_WIKIDATA
       AND sc.ID_PROPERTY = 'P4985'
...
INNER JOIN T_WC_WIKIDATA_EXTERNAL_ID_VALUE corrob ON corrob.ID_STATEMENT = sc.ID_STATEMENT
       AND corrob.VALUE_EXTERNAL_ID = P1.ID_PERSON
...
AND P1.ID_WIKIDATA REGEXP '^Q[0-9]+$'
AND P1.ID_WIKIDATA <> WP.ID_WIKIDATA
